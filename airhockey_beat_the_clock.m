...
figure('units','normalized','outerposition',[0 0 1 1],'WindowStyle','modal');
    ...
    [yyyyy,Fs] = wavread('hitwall.wav');
       ...
       [yyyyy1,Fs] = wavread('boo.wav');
            boo_sound=audioplayer(yyyyy1,Fs);
    ...
    r_striker = 300;
    striker_1 = rectangle('Position',[2000-r_striker,2000-r_striker, 2*r_striker,2*r_striker], 'Curvature',[1 1], 'FaceColor',[1 1 0]);
    ...
    %scoreboard
     [scoreboard blah blah111]= imread('scoreboard1.png');
    ...
    %game runs on button press to make timing accurate
    btnprs=waitforbuttonpress;
    tic
    %main game loop
        while con
        game_start=tic;        
        ...
        if time_elapsed>120
           ...
           if score<10
            text(-4000,5000,'You can do better.. Try again?','fontname','Kristen ITC','fontsize',40,'color',[0 1 1]);
           ...
           if score>=10
            text(-7500,5000,'You are a champ! Go again or Challenge a friend!','fontname','Kristen ITC','fontsize',40,'color',[0 0 1]);              
            buttongame = imread('beattheclockicon.png');
            btt = imresize(buttongame, [100 215]);
            h2 = uicontrol('Style','PushButton','Units','normalized',...
            'cdata', btt,'Position',[.44 .40 .159 .137]);
        ...
        mouseH(zx,:)=[mouseLoc(1) mouseLoc(3)];
                               
        %x and y cordinates of puck and striker
        x_of_striker=mouseLoc(1)-r_striker;
        x_of_puck=x-r_puck;
        y_of_striker=mouseLoc(3)-r_striker;
        y_of_puck=y-r_puck;
        ...
        set(striker_1,'Position',[mouseLoc(1)-r_striker mouseLoc(3)-r_striker 2*r_striker,2*r_striker ]);    
        set(puck,'position',[x-r_puck,y-r_puck,2.*r_puck,2.*r_puck])
        
        %score title
        time_remaining=120-floor(time_elapsed);
        score_string=num2str(score);
        string_time_remaining=num2str(time_remaining);
        top_text=char('Time Remaining: ',string_time_remaining);        
